...
import { QueryClient } from "cosmwasm";
...
// This creates a client for the CosmWasm chain, that can interact with contracts
export async function setupWasmQueryClient(): Promise<StargateClient> {
    const cosmwasm = await queryClient(wasmd);
    return cosmwasm;
}

// This creates a client for the CosmWasm chain, that can interact with contracts
export async function setupOsmosisQueryClient(): Promise<StargateClient> {
    // create apps and fund an account
    const cosmwasm = await queryClient(osmosis);
    return cosmwasm;
}
